...
func containsBytes(b []byte, c byte) bool {
	return bytes.IndexByte(b, c) != -1
}

func quoteWith(s *String, quote byte, ASCIIonly, graphicOnly bool) []byte {
	return appendQuotedWith(make([]byte, 0, 3*s.Len()/2), s, quote, ASCIIonly, graphicOnly)
}

func quoteRuneWith(r rune, quote byte, ASCIIonly, graphicOnly bool) []byte {
	return appendQuotedRuneWith(nil, r, quote, ASCIIonly, graphicOnly)
}
...
func unquote(in *String, unescape bool) (out []byte, rem *String, err error) {
	// Determine the quote form and optimistically find the terminating quote.
	if in.Len() < 2 {
		return nil, in, ErrSyntax
	}
	quote := in.buf[0]
	end := BytesString(in.buf[1:]).Index(quote)
	if end < 0 {
		return nil, in, ErrSyntax
	}
	...
	case '`':
		switch {
		case !unescape:
			out = in.buf[:end] // include quotes
		case !containsBytes(in.buf[:end], '\r'):
			out = in.buf[len("`") : end-len("`")] // exclude quotes
		default:
			buf := make([]byte, 0, end-len("`")-len("\r")-len("`"))
			for i := len("`"); i < end-len("`"); i++ {
				if in.buf[i] != '\r' {
					buf = append(buf, in.buf[i])
				}
			}
			out = buf
		}
		...
		return out, BytesString(in.buf[end:]), nil
	case '"', '\'':
		// Handle quoted strings without any escape sequences.
		if !containsBytes(in.buf[:end], '\\') && !containsBytes(in.buf[:end], '\n') {
			var valid bool
			switch quote {
			case '"':
				valid = utf8.Valid(in.buf[len(`"`) : end-len(`"`)])
			case '\'':
				r, n := utf8.DecodeRune(in.buf[len("'") : end-len("'")])
		...
		var buf []byte
		in0 := in
		in = BytesString(in.buf[1:]) // skip starting quote
		if unescape {
			buf = make([]byte, 0, 3*end/2) // try to avoid more allocations
		}
		for in.Len() > 0 && in.buf[0] != quote {
			// Process the next character,
			// rejecting any unescaped newline characters which are invalid.
			r, multibyte, rem, err := UnquoteChar(in.buf, quote)
			if in.buf[0] == '\n' || err != nil {
				return nil, in0, ErrSyntax
			}
			in = BytesString(rem)
		...
		if !(len(in.buf) > 0 && in.buf[0] == quote) {
			return nil, in0, ErrSyntax
		}
		in.RemoveIndexStr(1) // skip terminating quote

		if unescape {
			return buf, in, nil
		}
		return in0.buf[:in0.Len()-in.Len()], in, nil
	default:
		return nil, in, ErrSyntax
	}
}
